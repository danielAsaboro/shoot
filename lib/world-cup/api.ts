export async function fetchWorldCupSnapshot(params: {
  ...
  if (params.weights) searchParams.set("weights", JSON.stringify(params.weights));
  if (params.guardrails) searchParams.set("guardrails", JSON.stringify(params.guardrails));

  const url = `/api/world-cup/snapshot?${searchParams.toString()}`;
  const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) throw new Error(`World Cup snapshot failed: ${response.status}`);
  return response.json();
}

export async function fetchWorldCupLeaderboard(cupId: string, scenarioId: string) {
...
export async function fetchWorldCupBracket(scenarioId: string) {
  ...
  const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) throw new Error(`Bracket request failed: ${response.status}`);
  return response.json();
}

export async function postWorldCupSimulation(body: { weights: Record<string, number>; guardrails: Record<string, number> }) {
...
export async function fetchFullTournament(params: {
  ...
  const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) throw new Error(`Full tournament request failed: ${response.status}`);
  return response.json();
}
